...
import { fixtureDoCanal, iniciarCanalAoVivo, statusDoCanal } from '@/server/live';
...
export async function GET(): Promise<NextResponse> {
  ...
  const fixtureId = fixtureDoCanal();
  ...
  if (fixtureId) {
    try {
      ...
      const [ev] = await db.query(
        'select count(*)::int as n from match_events where fixture_id = $1',
        [fixtureId],
      );
      const [od] = await db.query(
        'select count(*)::int as n from match_odds where fixture_id = $1',
        [fixtureId],
      );
      banco = { matchEvents: Number(ev?.n ?? 0), matchOdds: Number(od?.n ?? 0) };
    } catch (e) {
